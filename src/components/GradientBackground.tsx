...
const GradientBackground = () => {
  useEffect(() => {
    ...
    const update = () => {
      ...
      // Movimento original, leve órbita em torno do centro
      const amplitude = 10;
      const x = 50 + Math.sin(p * Math.PI * 2) * amplitude;
      const y = 50 + Math.cos(p * Math.PI * 2) * amplitude;
    ...
    if (reduceMotion) {
      // Mantém valores padrão sem movimento quando o usuário prefere menos animação
